...
def _write_dependency_section(f, changeset_section):
    if changeset_section.changed:
        ...
        rows = sorted(
            [
                {"Package": p, "Previous Version": prev, "Current Version": curr, "Change Type": _classify_bump(prev, curr)}
...
def generate_change_log(target_version: Version, image_config):
    ...
    has_indirect = any([changeset.indirect.changed, changeset.indirect.added, changeset.indirect.removed])
